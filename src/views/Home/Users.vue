<template>
  <div>
    ...
    <!-- 修改用户的对话框 -->
    <el-dialog title="修改用户" :visible.sync="editDialogVisible" width="50%" center @close="editDialogClosed">
      <!-- 内容主体区域 -->
...
export default {
  data () {
    // 自定义校验规则
    // value: 为待校验的表单项的值
    // callback: 成功或失败的回调方法,执行不传参数代表校验成功
    ...
    return {
      ...
      addUserFormRules: {
        username: [
          ...
          { min: 3, max: 10, message: '用户名在三到十个字符之间', trigger: 'blur' }
        ],
        password: [
          { required: true, message: '请输入密码', trigger: 'blur' },
          { min: 6, max: 15, message: '用户名在六到十五个字符之间', trigger: 'blur' }
        ],
        email: [
  ...
  methods: {
    ...
    async deleteUserById (id) {
      ...
      // 如果用户点击了确认,则返回字符串confirm
      // 如果用户点击了取消,则返回字符串cancle
